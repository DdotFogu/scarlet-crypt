...
static func spawn_dmg_popup(attack: Attack, parent: Node):
	...
	var side_offset = 50 * Global.get_side_relation(parent, attack.killer)
	...
	scale_tween.tween_property(popup, "global_position", popup.global_position + Vector2(Global.get_side_relation(parent, attack.killer), -60), 0.25).set_trans(Tween.TRANS_BACK)
